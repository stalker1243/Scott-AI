...
def _ensure_data_dir(base: str) -> str:
    """
    Создать папку для данных: профили, макросы, напоминания, история чата.

    В дистрибутив она не входит намеренно — там лежат данные конкретной
    машины, — а модули пишут в неё напрямую, через open(...). Без создания
    сохранение молча проваливалось бы у каждого, кто поставил Scott
    установщиком: except в save_profiles печатает ошибку в консоль, которую
    никто не видит, и профили просто не сохраняются.
    """
...
async def _warmup_models() -> None:
    ...
    # миллисекунд, а из кэша та же фраза достаётся за треть миллисекунды. Эти
    # реплики звучат после каждой команды — «Готово, сэр», «Выполнено
    # успешно», — так что плата набегает.
    #
    # Кэш на диске переживает перезапуск: платится это один раз за всю жизнь
    ...
    try:
        ...
        print(f"⚠️ Прогрев готовых реплик не удался: {e}")
