...
# ==================== Вырезание служебного начала ====================

@pytest.mark.parametrize("text,expected", [
    ("какие процессы запущены", "процессы запущены"),
    ("что такое фотосинтез", "фотосинтез"),
    ("как работает двигатель", "работает двигатель"),
    ("сколько времени", "времени"),
    ("процессы запущены", "процессы запущены"),
])
def test_leading_phrase_stripped_by_word(модули, text, expected):
    """
    Служебное начало вырезается целиком, а не по первым буквам.

    Простой `startswith` вырезал «как» из «какие», оставляя «ие процессы
    запущены» — Scott искал процессы с таким именем. Здесь же вторая тонкость:
    длинные выражения проверяются первыми, иначе «что» вырезается раньше, чем
    «что такое», и в теме вопроса остаётся висеть слово «такое».
    """
    словарь = модули["vocabulary"]
    начала = ("что такое", "что", "как", "какие", "сколько", "покажи")

    assert словарь.strip_leading(text, начала) == expected


def test_leading_phrase_equal_to_whole_text(модули):
    """Фраза целиком из служебного слова оставляет пустую тему."""
    assert модули["vocabulary"].strip_leading("покажи", ("покажи",)) == ""
