...
# ==================== Уверенность считается по совпадению ====================

def test_confidence_does_not_depend_on_list_length(parser):
    """
    Уверенность зависит от того, что совпало, а не от длины списка синонимов.

    Считалось как очки, поделённые на половину длины списка. Из этого следовали
    две неприятности. Первая: каждый добавленный синоним молча снижал
    уверенность для всего типа, и тип, который раньше выигрывал, начинал
    проигрывать — ловушка того же рода, что уже стоила трёх дефектов. Вторая:
    число ничего не значило, и за одинаково ложные совпадения «поисковик»
    получал 0.21, а «сайтостроение» — 0.67.

    Теперь одинаковые по точности совпадения дают одинаковую уверенность,
    сколько бы синонимов ни лежало в списке рядом.
    """
    одно_слово = parser.parse("закрой дискорд").confidence
    другое_слово = parser.parse("открой браузер").confidence

    assert одно_слово == другое_слово, "одинаковые совпадения — разная уверенность"


def test_longer_match_wins(parser):
    """
    Многословное совпадение точнее одного слова.

    «Открой сайт гугл» — просьба открыть сайт, а не программу «сайт гугл»: во
    фразе совпадают и «открой», и «открой сайт», и выбрать нужно второе.
    """
    parsed = parser.parse("открой сайт гугл")
    assert parsed.command_type == "open_website", f"-> {parsed.command_type}"
    assert parsed.confidence > parser.parse("открой браузер").confidence
...
def test_no_match_inside_other_words(parser, phrase):
    """
    Синоним не должен находиться внутри обычного слова.

    Совпадения искались простым вхождением: «поиск» находился внутри
    «поисковика», «сайт» — внутри «сайтостроения». Первое превращало вопрос в
    поиск по интернету, второе — в попытку открыть сайт с таким названием.
    """
    parsed = parser.parse(phrase)
    assert parsed.command_type == "unknown", f"«{phrase}» -> {parsed.command_type}"


def test_folder_is_not_a_file_operation(parser):
    """
    Открыть папку — не операция над файлом.

    «Открой папку» лежало в списке файловых операций, и при точном подсчёте
    это двухсловное совпадение стало перебивать запуск: «открой папку загрузки»
    уходило в ветку копирования-удаления вместо открытия папки.
    """
    assert parser.parse("открой папку загрузки").command_type != "file_operation"
    assert parser.parse("удали файл отчёт").command_type == "file_operation"
